...
export default function LinkAccountPage() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (!j.found) return;
      if (j.expired) { setExpired(true); if (pollRef.current) clearInterval(pollRef.current); return; }
      if (j.claimed && j.claimedBy) {
        localStorage.setItem("brainbot:tid", String(j.claimedBy));
        setClaimed(true);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-ink-900 via-ink-800 to-ink-900 text-white flex items-center px-4">
      <div className="glass mx-auto w-[90vw] max-w-xl rounded-2xl shadow-glass p-8 text-center">
        ...
        <p className="text-steel-300 mt-2">Open Telegram and send the code below to <b>@brainbotafrica_bot</b> using <code>/link &lt;code&gt;</code>.</p>
