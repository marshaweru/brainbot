...
export function registerStart(bot: Telegraf<Context>) {
  bot.start(async (ctx) => {
    ...
    const tid = String(ctx.from?.id); // 🔄 always store as string
    ...
    if (maybeJson?.wid && tid) {
      try {
        ...
        const plan =
          typeof maybeJson.plan === "string" ? maybeJson.plan : undefined;
        ...
        await ctx.reply(
          already
            ...
            : "✅ Linked successfully! You can now upload your answers and type /finish when done."
        );
        await ctx.reply(
        ...
        console.error(
          "Base64url linking failed; falling back to confirmLink:",
          err
        );
      }
    }
